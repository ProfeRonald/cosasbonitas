...
/*
	...
	*/
	...
	$(document).on('click', '.boped', function (event) { 	 
		...
		$('#cerrarped').attr('class', 'btn btn-secondary');
	...
	$(document).on('click', '#numped', function (event) {
  	...
  	$(this).prop('disabled', true);
		...
		$.ajax({
  		...
  		$("#pedidos img").attr('data-original-title', 'Tienes <strong>' + pc + '</strong> pedidos en proceso de entrega');
  		if(pc == 0){
  		$("#pedidos").hide('slow');
  		}else{
  		$("#pedidos").show('slow');  		
  		...
  		setTimeout(function(){
  		var nped = Number($('#npedidos strong').text()) - 1;
  		$('#npedidos strong').text(nped);
  		$('#'+idped).attr('class', 'd-none');
  		$('#cerrarped').trigger('click');
  		$('#cerrarped').attr('class', 'btn btn-secondary cerrarimg');
...
$(document).on('click', '#gps', function (event) {
	...
	$(document).on('click', '#carrito, .cant_facq, .cant_facp', function (event) {
		...
		$.ajax({
  			...
  			dataType: 'json',
				data: {noc:noc, cod:cod, cant:cant},
			})
  		.done(function(fac){
...
$(document).on('click', '#cerrardatos, .cerrardatos', function (event) {
        ...
        return false;
  		...
  		$('#carritoModal').modal('hide');
  		$("#carrito").hide('slow');
  		window.location = 'index.php?pagar=' + pdo;
  		},2500);
